...
class lct_controller():
    ...
    def display_data(self, event, word_object):
        self.main_win.word_header[0].configure(text=word_object.attributes["word"])
        self.main_win.word_header[0].bind('<Double-Button-1>', lambda event: self.editor_switch(event, self.main_win.word_header, False, word_object))


    def editor_switch(self, event, element, switch, word_object):
        print("SWITCH")
        if switch == True:
            self.vocab.update_word(word_object.attributes["word_id"], self.main_win.word_header[3], element[switch].get())
        element[switch].grid_forget()
        element[not switch].grid(column=element[2][0], row=element[2][1], sticky=element[2][2])
        try:
            element[not switch].delete(0, tk.END)
            element[not switch].insert(0,element[switch]["text"])
            element[not switch].bind("<Return>", lambda event:self.editor_switch(event, element, True, word_object))
        except:
            pass
        self.load_vocabulary
